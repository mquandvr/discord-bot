...
import {
  convertStrToTimetamp,
  convertYMDStrToTimetamp,
} from "../../../utils/date.js";

import { GoogleGenerativeAI } from "@google/generative-ai";
import {
  COLLECTION_WUWE_CHANNEL,
  COLLECTION_WUWE_NEWS,
  ...
  if (subcommand === "choose") {
    if (date && !isNaN(new Date(date))) {
      ...
      await interaction.editReply({
        ephemeral: true,
        content: "Wrong format date (yyyyMMdd)!.",
      });
      return false;
    }
  }
  ...
  try {
    ...
    if (subcommand === "schedule") {
      ...
      const content = enabled
        ? "Schedule added! Bot will check and post news updates every 1 hour/day."
...
const retriveContent = async (channel, guild, date) => {
  ...
  const dataPosted = await connection
    ...
    .findByCondition();
  const dataArticleFilters = dataArticles.filter(
    (x) =>
      convertStrToTimetamp(x.createTime) === newDate.getTime() &&
    ...
    for (const dataArticle of dataArticleFilters) {
      ...
      if (dataArticleDetail) {
        ...
        for (const [index, content] of array.entries()) {
          ...
          if (
            dataMerge.length + content.length > CONTENT_MAX_LENGTH ||
        ...
        await connection
          .setData({
            articleId: dataArticleDetail?.articleId,
        ...
        log.info(
          `data ${dataArticleDetail?.articleId} sent to server/channel ${guild.name}/${channel.name}`,
        );
      }
    }
  } else {
